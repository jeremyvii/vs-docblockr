...
export class Lexer {
  ...
  protected attrs(): boolean {
    ...
    if ('(' === this.input.charAt(0)) {
      ...
      this.tokens.push(this.tokenize(LexerToken.startAttributes));
      ...
      const isEndOfAttribute = (j: number): boolean => {
        ...
        if (loc === 'key') {
          ...
          if (!val || state.isNesting() || state.isString()) {
            return false;
          }
          // Find the first non-whitespace character
          if (whitespaceRe.test(attrStr[j])) {
      ...
      for (let i = 0; i <= attrStr.length; i++) {
        ...
        if (isEndOfAttribute.call(this, i)) {
          ...
          const tok = this.tokenize(LexerToken.attribute);
          tok.name = key;
      ...
      // Push ending token
      this.tokens.push(this.tokenize(LexerToken.endAttributes));
  ...
  protected code(): boolean {
    ...
    if (matches) {
      ...
      // Create code token
      const tok = this.tokenize(LexerToken.code, code);
  ...
  protected colon(): boolean {
    // Scan for colon type
    const tok = this.scan(/^: +/, LexerToken.colon);
  ...
  protected eos(): boolean {
    ...
    this.tokens.push(this.tokenize(LexerToken.eos));
   ...
   * @param   {LexerToken}  type   Type to scan for
   *
   * @return  {Lexed}
   */
  protected scan(regex: RegExp, type: LexerToken): ILexed {
  ...
  protected tag(): boolean {
    ...
    if (matches) {
      ...
      this.consume(length);
      // Create tag token
      const token = this.tokenize(LexerToken.tag, name);
  ...
  protected text(): boolean {
    ...
    const tok = this.scan(/^(?:\| ?| )([^\n]+)/, LexerToken.text) ||
      this.scan(/^\|?( )/, LexerToken.text);
    // Check token was created
    if (tok) {
      // Push text token to list
      this.tokens.push(this.tokenize(LexerToken.text, tok.val));
  ...
  protected tokenize(type: LexerToken, val?: string): ILexed {
    const token: ILexed = {
      col: this.column,
      line: this.line,
      type,
